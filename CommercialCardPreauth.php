...
<fieldset>
    ...
    <label style="font-weight:bold;">Servis Adı &nbsp; :   &nbsp; </label> commercialcardpreauth<br>
...
<?php
    ...
    $request = new CommercialCardPreauthRequest();
    ...
    $request->Transaction->Type="commercialcardpreauth";
    $request->CurrencyCode="949";
    $request->MotoInd="N";
 

    $request->Hash=Helper::ComputeHash($request,$settings);
    $response = CommercialCardPreauthRequest::execute($request,$settings); //CommercialCardPreauthRequest servisi başlatılması için gerekli servis çağırısını temsil eder.
    print "<h3>Sonuç:</h3>";
	echo "<pre>";

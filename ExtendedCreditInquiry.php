...
<h2>Tüketici Kredisi Sorgulama </h2>
<br />
<fieldset>

    <legend><label style="font-weight:bold;width:250px;">Terminal Bilgileri</label></legend>
    <label style="font-weight:bold;">Servis Adı &nbsp; :   &nbsp; </label> Extendedcreditinq<br>
...
<?php
    ...
    $request = new ExtendedCreditInquiryRequest();
    ...
    $request->Order->OrderID=Helper::GenerateOrderId();
    ...
    $request->Transaction->Type="extendedcreditinq";
    $request->CurrencyCode="949";
    $request->MotoInd="H";
    $request->InstallmentCnt=$_POST["installmentCnt"];
    $request->CardholderPresentCode = "0";

    $request->Hash=Helper::ComputeHash($request,$settings);
    $response = ExtendedCreditInquiryRequest::execute($request,$settings); //ExtendedCreditInquiryRequest servisi başlatılması için gerekli servis çağırısını temsil eder.
    print "<h3>Sonuç:</h3>";
	echo "<pre>";

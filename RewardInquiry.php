...
<fieldset>
    ...
    <label style="font-weight:bold;">Servis Adı &nbsp; :   &nbsp; </label> Rewardinq<br>
...
<?php
    ...
    $request = new RewardInquiryRequest();
    ...
    $request->Transaction->Type="rewardinq";
    $request->CurrencyCode="949";
    $request->MotoInd="H";
    $request->CardholderPresentCode = "0";

    $request->Hash=Helper::ComputeHash($request,$settings);
    $response = RewardInquiryRequest::execute($request,$settings); //RewardInquiryRequest servisi başlatılması için gerekli servis çağırısını temsil eder.
    print "<h3>Sonuç:</h3>";
	echo "<pre>";

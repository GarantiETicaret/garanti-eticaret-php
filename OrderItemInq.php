...
<?php
    ...
    $request = new OrderItemInqRequest();
    ...
    $request->Transaction->Type="orderiteminq";
    $request->CurrencyCode="949";
  
    $request->Hash=Helper::ComputeHash($request,$settings);

    $response = OrderItemInqRequest::execute($request,$settings); //OrderItemInqRequest servisi başlatılması için gerekli servis çağırısını temsil eder.
    print "<h3>Sonuç:</h3>";
	echo "<pre>";

...
<fieldset>
    ...
    <label style="font-weight:bold;">Servis Adı &nbsp; :   &nbsp; </label> Dccinq<br>
...
<?php
    ...
    $request->Transaction->Type="dccinq";
    $request->CurrencyCode="949";
    $request->MotoInd="H";
    $request->CardholderPresentCode = "0";

    $request->Hash=Helper::ComputeHash($request,$settings);
    $response = DccInquiryRequest::execute($request,$settings); //DccInquiryRequest servisi başlatılması için gerekli servis çağırısını temsil eder.
    print "<h3>Sonuç:</h3>";
	echo "<pre>";
